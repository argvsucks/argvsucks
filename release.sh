...
source /home/davi/.cache/pypoetry/virtualenvs/argvsucks-s1iaLgEs-py3.10/bin/activate
pip install .
#################################################################################
#################################################################################
rm docs -rf
poetry run black -l200 src/ tests/
poetry run pdoc --html --force argvsucks -o docs
mv docs/argvsucks/* docs/
rm docs/argvsucks -rf
git add docs
